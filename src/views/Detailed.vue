<template>
  <div class="detailed">
    <div class="container">
      <div class="header-wrapper box d-flex v-center h-between">
        <router-link to="/">Go back</router-link>
        <span class="download-icon d-flex v-center" @click.stop="downloadFile">
          Download file
          <svg
            class="ml-1"
            version="1.1"
        ...
        <h1 class="neu-morphism-shadow" v-if="name.trim() !== ''">{{setkeyNormalizer(name)}}</h1>
      </div>
      <List :list="details" />
    </div>
  </div>
...
import {
  characterEscaperDecoder,
  keyNormalizer,
  downloadObjectAsJson
} from "./../custom_methods/swivel_methods";
export default {
  name: "detailed",
  data() {
    return {
  ...
  methods: {
    ...
    characterEscaperDecoderMethod(string) {
      characterEscaperDecoder(string);
    },
    downloadFile() {
      downloadObjectAsJson(this.details, this.name);
    }
  },
  mounted() {
    this.name = characterEscaperDecoder(
      this.decodeName(this.$route.params.name)
    );
    this.type = characterEscaperDecoder(
      this.decodeType(this.$route.params.type)
    );
    if (this.type === "ticket_subject") {
      this.details = tickets.find(o => {
        return o.subject.toLowerCase() === this.name;
      });
    } else if (this.type === "username") {
      this.details = users.find(o => {
        return o.name.toLowerCase() === this.name;
      });
    } else if (this.type === "organization_name") {
      this.details = organizations.find(o => {
        return o.name.toLowerCase() === this.name;
      });
    }
  }
